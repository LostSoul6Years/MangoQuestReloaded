...
public class QuestObjectBreedMob extends NumerableObject implements EditorObject
{
	...
	public void setMythicMob(String m) {
		if (!Main.getHooker().getMythicMobsAPI().isMythicMobPresent(m))
			return;
		mtmMob = m;
		customName = Main.getHooker().getMythicMobsAPI().getMythicPlaceholderedName(m);
		if (Main.getHooker().getMythicMobsAPI().getMythicEntityTypeRaw(m).contains("BABY"))
			isBaby = true;
		type = EntityType.valueOf(Main.getHooker().getMythicMobsAPI().getMythicEntityTypeRaw(m).replace("BABY_", "").toUpperCase());
	}
	...
	public boolean isMythicObject() {
		return Main.getHooker().getMythicMobsAPI().isMythicMobPresent(mtmMob);
	}
	...
	public TextComponent toTextComponent(Player p,boolean isFinished) {
		if (hasCustomName())
			...
			return super.toTextComponent(p,ChatColor.stripColor(I18n.locMsg(null,"QuestObject.BreedMob")), isFinished, type, amount);
	}

	@Override
	public String toDisplayText() {
		if (hasCustomName())
			return I18n.locMsg(null,"QuestObject.BreedMob", QuestChatManager.translateColor(customName), Integer.toString(amount));
		else
			return I18n.locMsg(null,"QuestObject.BreedMob", QuestUtil.translate(type), Integer.toString(amount));
	}
	...
	public void formatEditorPage(Player p,FlexibleBook page, int stage, int obj) {
		if (Main.getHooker().hasMythicMobEnabled()) {
			...
			if (isMythicObject())
				page.add(new InteractiveText(p,Main.getHooker().getMythicMobsAPI().getMythicPlaceholderedName(mtmMob) + "(" + mtmMob + ")"));
			else
		...
		page.add(new InteractiveText(p,I18n.locMsg(null,"QuestEditor.Edit")).clickCommand("/mq e edit object " + stage + " " + obj + " mobname"))
				.changeLine();
		page.add(I18n.locMsg(null,"QuestEditor.MobType") + QuestUtil.translate(type));
	...
	public boolean load(QuestIO config, String path) {
		if (config.getString(path + "MythicMob") != null) {
			...
			mtmMob = id;
			customName = Main.getHooker().getMythicMobsAPI().getMythicPlaceholderedName(id);
			String typeName = Main.getHooker().getMythicMobsAPI().getMythicEntityTypeRaw(id).toUpperCase();
	...
	public boolean receiveCommandInput(Player sender, String type, String obj) {
		switch (type) {
			...
			case "mtmmob":
				...
				if (Main.getHooker().getMythicMobsAPI().getMythicEntityTypeRaw(obj).contains("baby")) {
					isBaby = true;
					setType(EntityType.valueOf(Main.getHooker().getMythicMobsAPI().getMythicEntityTypeRaw(obj).replace("baby_", "").toUpperCase()));
				} else
					setType(EntityType.valueOf(Main.getHooker().getMythicMobsAPI().getMythicEntityTypeRaw(obj).toUpperCase()));
				break;
			default:
