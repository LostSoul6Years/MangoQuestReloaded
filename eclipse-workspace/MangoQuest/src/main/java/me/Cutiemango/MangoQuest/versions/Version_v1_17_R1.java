...
public class Version_v1_17_R1 implements VersionHandler {
	...
	public void sendTitle(Player p, Integer fadeIn, Integer stay, Integer fadeOut, String title, String subtitle) {
		...
		Method m = null;
		try {
			m = ((CraftPlayer)cp).getHandle().b.getClass().getDeclaredMethod("sendPacket", Packet.class);
		...
		if (title != null) {
			...
			Method m1;
			try {
				m1 = ((CraftPlayer)cp).getHandle().b.getClass().getDeclaredMethod("sendPacket", Packet.class);
	...
	public void openBook(Player p, TextComponent... texts) {
		...
		Method m1;
		try {
			m1 = ((CraftPlayer)p).getHandle().b.getClass().getDeclaredMethod("sendPacket", Packet.class);
			m1.setAccessible(true);
