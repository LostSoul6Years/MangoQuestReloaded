...
public class QuestEditorManager
{
	...
	public static void editQuest(Player p) {
		...
		settings.add(I18n.locMsg(p,"QuestEditor.QuestSettings")).changeLine();
		settings.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.IsQuitable")).clickCommand("/mq e edit quit " + !q.isQuitable())
				.showText(I18n.locMsg(p,"QuestEditor.IsQuitable.ShowText." + !q.isQuitable())));
		settings.add(I18n.locMsg(p,"QuestEditor." + q.isQuitable())).changeLine();
		settings.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.UsePermission")).clickCommand("/mq e edit perm " + !q.usePermission())
				.showText(I18n.locMsg(p,"QuestEditor.UsePermission.ShowText." + !q.usePermission())));
		settings.add(I18n.locMsg(p,"QuestEditor." + q.usePermission())).changeLine();
		settings.add(new InteractiveText(p,(I18n.locMsg(p,"QuestEditor.WorldLimit"))).clickCommand("/mq e edit world")
				.showText(I18n.locMsg(p,"QuestEditor.WorldLimit.ShowText")));
		if (q.hasWorldLimit())
			settings.add(q.getWorldLimit().getName()).changeLine();
		else
			settings.add(I18n.locMsg(p,"QuestEditor.NotSet")).changeLine();

		settings.changeLine();

		// Display settings
		settings.add(new InteractiveText(p,I18n.locMsg(p,"QuestVisibility.OnTake")).clickCommand("/mq e edit vis take " + !vs.displayOnTake())
				.showText(I18n.locMsg(p,"QuestVisibility." + !vs.displayOnTake())));
		settings.add(I18n.locMsg(p,"QuestEditor." + vs.displayOnTake())).changeLine();
		settings.add(new InteractiveText(p,I18n.locMsg(p,"QuestVisibility.OnProgress")).clickCommand("/mq e edit vis prog " + !vs.displayOnProgress())
				.showText(I18n.locMsg(p,"QuestVisibility." + !vs.displayOnProgress())));
		settings.add(I18n.locMsg(p,"QuestEditor." + vs.displayOnProgress())).changeLine();
		settings.add(new InteractiveText(p,I18n.locMsg(p,"QuestVisibility.OnFinish")).clickCommand("/mq e edit vis finish " + !vs.displayOnFinish())
				.showText(I18n.locMsg(p,"QuestVisibility." + !vs.displayOnFinish())));
		settings.add(I18n.locMsg(p,"QuestEditor." + vs.displayOnFinish())).changeLine();
		settings.add(new InteractiveText(p,I18n.locMsg(p,"QuestVisibility.OnInteraction"))
		...
		settings.add(I18n.locMsg(p,"QuestEditor." + vs.displayOnInteraction())).changeLine();

		// Time Limit
		settings.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.IsTimeLimited")).clickCommand("/mq e edit limit " + !q.isTimeLimited())
				.showText(I18n.locMsg(p,"QuestEditor.IsTimeLimited.ShowText." + !q.isTimeLimited())));
		settings.add(I18n.locMsg(p,"QuestEditor." + q.isTimeLimited())).changeLine();
		if (q.isTimeLimited()) {
			settings.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.TimeLimit")).clickCommand("/mq e edit timelimit")
		...
		p2.add(I18n.locMsg(p,"QuestEditor.ReqEventStageInfo")).changeLine();
		p2.changeLine();
		p2.add(I18n.locMsg(p,"QuestEditor.Requirement")).changeLine();
		p2.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.GoToEditPage")).clickCommand("/mq e edit req")
				.showText(I18n.locMsg(p,"QuestEditor.Requirement.ShowText"))).changeLine();
		p2.changeLine();
		p2.add(I18n.locMsg(p,"QuestEditor.QuestEvent")).changeLine();
		p2.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.GoToEditPage")).clickCommand("/mq e edit evt")
				.showText(I18n.locMsg(p,"QuestEditor.Event.ShowText"))).changeLine();
		p2.changeLine();
		p2.add(I18n.locMsg(p,"QuestEditor.QuestStage")).changeLine();
		p2.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.GoToEditPage")).clickCommand("/mq e edit stage")
				.showText(I18n.locMsg(p,"QuestEditor.QuestStage.ShowText"))).changeLine();

		QuestBookPage p3 = new QuestBookPage();
		p3.add(I18n.locMsg(p,"QuestEditor.Outline")).changeLine();
		for (String out : q.getQuestOutline()) {
			p3.add(out).changeLine();
		}
		p3.changeLine();
		p3.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.Edit")).clickCommand("/mq e edit outline")).changeLine();

		QuestBookPage p4 = new QuestBookPage();
		p4.add(I18n.locMsg(p,"QuestEditor.Reward")).changeLine();

		p4.add(I18n.locMsg(p,"QuestEditor.RewardMoney", Double.toString(q.getQuestReward().getMoney())));
		p4.add(new InteractiveText(p," " + I18n.locMsg(p,"QuestEditor.Edit")).clickCommand("/mq e edit reward money")
				.showText(I18n.locMsg(p,"QuestEditor.RewardMoney.ShowText"))).changeLine();

		p4.add(I18n.locMsg(p,"QuestEditor.RewardExp", Integer.toString(q.getQuestReward().getExp())));
		p4.add(new InteractiveText(p," " + I18n.locMsg(p,"QuestEditor.Edit")).clickCommand("/mq e edit reward exp")
				.showText(I18n.locMsg(p,"QuestEditor.RewardExp.ShowText"))).changeLine();

		if (Main.getHooker().hasSkillAPIEnabled()) {
			p4.add(I18n.locMsg(p,"QuestEditor.RewardRPGExp", Integer.toString(q.getQuestReward().getSkillAPIExp())));
			p4.add(new InteractiveText(p," " + I18n.locMsg(p,"QuestEditor.Edit")).clickCommand("/mq e edit reward saexp")
					.showText(I18n.locMsg(p,"QuestEditor.RewardRPGExp.ShowText"))).changeLine();
		}

		if (Main.getHooker().hasQuantumRPGEnabled()) {
			p4.add(I18n.locMsg(p,"QuestEditor.RewardRPGExp", Integer.toString(q.getQuestReward().getQRPGExp())));
			p4.add(new InteractiveText(p," " + I18n.locMsg(p,"QuestEditor.Edit")).clickCommand("/mq e edit reward qrpgexp")
					.showText(I18n.locMsg(p,"QuestEditor.RewardRPGExp.ShowText"))).changeLine();
		}

		p4.add(I18n.locMsg(p,"QuestEditor.RewardFriendPoint"));
		p4.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.Edit")).clickCommand("/mq e edit reward fp")
		...
		if (q.getQuestReward().hasFriendPoint()) {
			for (Integer n : q.getQuestReward().getFriendPointMap().keySet()) {
				...
				if (npc == null)
					continue;
				p4.add("- ");
				p4.add(new InteractiveText(p,"").showNPCInfo(npc));
				p4.add("&0 " + q.getQuestReward().getFriendPointMap().get(n) + " " + I18n.locMsg(p,"QuestEditor.Point"));
				p4.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.Remove")).clickCommand("/mq e remove reward fp " + n));
				p4.changeLine();
			}
		}

		p4.add(I18n.locMsg(p,"QuestEditor.RewardCommand"));
		p4.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.Add")).clickCommand("/mq e addnew reward command")
		...
		if (q.getQuestReward().hasCommand()) {
			...
			for (String s : q.getQuestReward().getCommands()) {
				p4.add(new InteractiveText(p,"- &0" + I18n.locMsg(p,"QuestEditor.Command") + "(" + counter + ")").showText("&f/" + s));
				p4.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.Edit")).clickCommand("/mq e edit reward command " + (counter - 1)));
				p4.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.Remove")).clickCommand("/mq e remove reward command " + (counter - 1)));
				p4.changeLine();
				counter++;
			}
		}
		p4.add(I18n.locMsg(p,"QuestEditor.RewardItem"));
		p4.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.Edit")).clickCommand("/mq e edit reward item")
				.showText(I18n.locMsg(p,"QuestEditor.RewardItem.ShowText"))).changeLine();

		QuestBookPage p5 = new QuestBookPage();
		p5.add(I18n.locMsg(p,"QuestEditor.SaveAndExit")).changeLine();
		p5.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.SyncSevAndLoc")).clickCommand("/mq e sa").showText(I18n.locMsg(p,"QuestEditor.WarnSave")))
				.changeLine();
		p5.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.SyncSev")).clickCommand("/mq e sl").showText(I18n.locMsg(p,"QuestEditor.WarnSave")))
				.changeLine();
		p5.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.SyncLoc")).clickCommand("/mq e sc").showText(I18n.locMsg(p,"QuestEditor.WarnSave")))
				.changeLine();
		p5.changeLine();
		p5.add(new InteractiveText(p,I18n.locMsg(p,"QuestEditor.QuitEditor")).clickCommand("/mq e exit")
				.showText(I18n.locMsg(p,"QuestEditor.ExitEditor.ShowText"))).changeLine();
