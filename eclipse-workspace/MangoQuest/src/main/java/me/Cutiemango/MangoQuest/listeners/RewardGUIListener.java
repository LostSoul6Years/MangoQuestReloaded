...
public class RewardGUIListener
{
	public static void onInventoryClick(InventoryClickEvent e) {
		...
		if (e.getCurrentItem() != null) {
			...
			if (inv.getTitle().contains(QuestChatManager.translateColor(I18n.locMsg(p,"QuestReward.RewardTitle")))) {
				...
				p.closeInventory();
			} else if (inv.getTitle().contains(QuestChatManager.translateColor(I18n.locMsg(p,"QuestReward.RewardEditTitle")))) {
				if (QuestGUIItem.isGUIItem(e.getCurrentItem())) {
					if (e.getCurrentItem().getType().equals(Material.ANVIL)&&e.getRawSlot()==26) {
						...
						return;
					} else if (e.getCurrentItem().getType().equals(Material.WRITABLE_BOOK)&&e.getRawSlot()==18) {
						p.closeInventory();
						QuestEditorManager.editQuest(p);
						return;
					} else if (e.getCurrentItem().getType().equals(Material.PLAYER_HEAD)&&e.getRawSlot()==0) {
						p.closeInventory();
	...
	public static void onInventoryClose(InventoryCloseEvent e) {
		...
		InventoryView inv = e.getView();
		if (inv.getTitle().contains(QuestChatManager.translateColor(I18n.locMsg(p,"QuestReward.ChoiceEditTitle")))) {
